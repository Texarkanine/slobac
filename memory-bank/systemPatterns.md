...
SLOBAC's canonical artifact is the per-smell manifesto, with `planning/VISION.md` as the product brief and `planning/research/` as the prior-art survey that justifies the positioning. The repo carries a single runtime audit skill at `skills/audit/` that orchestrates three subagent workflows bundled at `skills/audit/references/subagents/` (scout, batch, cross-suite). The skill bundle at `skills/audit/references/docs/` contains the **full manifesto** — principles, glossary, workflows, taxonomy entries, and site navigation. There is one document per smell; forking is structurally impossible. Manifesto-independence is enforced **structurally** — the skill bundle *is* the manifesto's home. The rendered site (properdocs) builds directly from this directory; no wrappers, no snippet indirection. Module boundaries are contained within the single `audit` skill: the orchestrator (`slobac-audit`) owns all shared references (taxonomy, format specs, report template, subagent workflows). All content is self-contained under `skills/audit/` — no `../` escapes, no sibling skill directories.
