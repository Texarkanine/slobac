...
### Rework requirement R1 — delegate slug enumeration to taxonomy content; impose slug-only invocation contract

The set of supported slugs is now **structurally** the set of files at `references/docs/taxonomy/*.md` (excluding `README.md`). The duplicate enumeration in SKILL.md is purely a mirror that drifts. Concurrently, the orchestrator's **fuzzy phrase-to-slug resolution mechanism is removed**: operators must invoke `/slobac-audit` with explicit slug(s); ambiguous phrasing is refused, not silently resolved.

- Drop the `Supported smells (N)` table from `skills/slobac-audit/SKILL.md` Step 2. Replace with an instruction that the supported set is enumerated by the existence of `references/docs/taxonomy/<slug>.md`.
- **Impose slug-only invocation contract.** SKILL.md Step 2 explicitly: (a) requires operators to invoke by explicit slug; (b) refuses non-slug or fuzzy-phrase requests with the supported-slug list and a prompt to re-invoke with explicit slugs (no silent phrase resolution); (c) refuses operator-named slugs whose taxonomy entry does not exist with the same supported-slug list; (d) preserves a small whitelist of unambiguous wildcard tokens (`all`, `everything`, unscoped) that resolve to the full supported-slug set — this is bulk-select, not phrase-to-slug fuzzy matching.
- **Move operator-phrase content to the taxonomy as `## Aliases`, but for human discoverability only.** The current SKILL.md operator-phrase map migrates to a uniform `## Aliases` section in each taxonomy entry. Its purpose flips from runtime-input to **search discoverability**: a human searching the published docs site for "mock system under test" lands on `tautology-theatre.md`, recognizes the right slug, then invokes `/slobac-audit` by slug. The orchestrator does **not** consume the section at runtime; SKILL.md does not reference it. Update `references/docs/taxonomy/README.md` shape SoT to require the new section and to state its purpose-and-audience explicitly (discoverability for humans landing on the docs site; not runtime input for the orchestrator).
...
- The human-facing supported-smells table in `skills/slobac-audit/README.md` is **not** in scope to delete — that's contributor documentation, not agent instructions, and a human-readable mirror is legitimate there. But its lead-paragraph count line must stop saying a specific number — replace `"Supports all 15 manifesto smells"` with phrasing equivalent to `"supports every smell defined in the manifesto"`. Same logic for `memory-bank/techContext.md`'s lead summary. The "Scope and non-goals" bullet in `slobac-audit/README.md` (currently asserts a manually-curated supported-slug table backs refusal) is rewritten to reflect structural enumeration.
