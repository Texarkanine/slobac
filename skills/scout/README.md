...
A subagent skill dispatched by [`slobac:audit`](../audit/README.md) to enumerate and measure a test suite before the orchestrator partitions it for batch assessment. The scout is fast, read-only, and never reads file contents deeply — it maps the suite's shape using filesystem operations and lightweight pattern matching.

## Relationship to slobac:audit

The scout is the first step in the audit orchestration pipeline. The orchestrator (`audit/SKILL.md`) launches the scout as a readonly subagent, receives the Suite Manifest, and uses it to decide how many batch assessors to launch and how to partition files between them.

```
slobac:audit (orchestrator)
  └── slobac:scout → Suite Manifest
      └── orchestrator uses manifest for partitioning
```

## Cross-skill references

The scout reads:
- `../audit/references/suite-manifest-format.md` — the output format spec
